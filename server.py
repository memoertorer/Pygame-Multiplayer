...
while ROOM_SIZE % PLAYER_SIZE != 0:
    PLAYER_SIZE += 1
#
#MAXIMUM NUMBER OF PLAYERS ALLOWED İS 4!
if TOTAL_PLAYERS > 4:
    TOTAL_PLAYERS = 4
#-----------------------------------------------------------------------
#Define Classes
...
class Player():
    ...
    def __init__(self, number):
        ...
        #Assign starting conditions that very with player number
        if self.number == 1:
            self.starting_x = 0
            self.starting_y = 0
    ...
    def set_player_info(self, player_info):
        '''Set the player info to the given info from client (coord and status flags)'''
...
class Game():
    ...
    def connect_players(self):
        ...
        #Only accept players if player count is less than total players
        while self.player_count < TOTAL_PLAYERS:
            ...
            #Create a new Player object for the new connected client
            self.player_count += 1
            ...
            #Alert all players of new player joining
            self.broadcast()
        ...
        #Maximum number of players reached. No longer accepting players
        print(f"{TOTAL_PLAYERS} in game. No longer accepting new players...")
    ...
    def broadcast(self):
        ...
        #Turn each player object into a dict and then into a string with json.dump
        for player_object in self.player_objects:
            player_json = json.dumps(player_object.__dict__)
            game_state.append(player_json)

        #Now that else has joined the game, send the game state to ALL players connected
    ...
    def ready_game(self, player, player_socket):
        ...
        #Wait till the given player has sent info signaling they are ready for the game to begin
        self.recieve_pregame_player_info(player, player_socket)

        #Reset the game
        self.reset_game(player)

        #If THIS is ready to play
        if player.is_ready:
            while True:
    ...
    def send_player_info(self, player, player_socket):
        ...
        #Send the player info to THIS player
    ...
    def recieve_game_player_info(self, player, player_socket):
        ...
        while player.is_playing:
            ...
            #Set the updates values for the given player
            player.set_player_info(player_info)

            #Process the new game state to see if a player scored points etc.
            self.process_game_state(player, player_socket)
        
        #Game has finished, monitor for the restart of the game.
        ready_thread = threading.Thread(target=self.ready_game, args=(player, player_socket,))
        ready_thread.start()


    def process_game_state(self, player, player_socket):
        '''Process the given player info and update the game state'''
        ...
        #Send the updated the game state back to THIS player
    ...
    def send_game_state(self, player_socket):
        ...
        #Turn each player object into a dict, then a string
        for player_object in self.player_objects:
            player_json = json.dumps(player_object.__dict__)
            game_state.append(player_json)
