...
#Define Classes
class Connection():
    '''A sockket connection class for players to connect to a server'''
    def __init__(self):
        '''Initialization of the Connection object'''
        self.encoder = "utf-8"
        self.header_length = 10

        #Create a socket anc connect
...
class Player():
    ...
    def __init__(self, connection):
        ...
        #Recieve the player info from the server
    ...
    def update(self):
        ...
        #Create a rect to char player movement
        ...
        if self.is_playing:
            if (keys[pygame.K_UP] or keys[pygame.K_w]) and player_rect.top > 0:
                self.dx = 0
                self.dy = -1 * self.size
            elif (keys[pygame.K_DOWN] or keys[pygame.K_s]) and player_rect.bottom < WINDOW_HEIGHT:
                self.dx = 0
                self.dy = 1 * self.size
            elif (keys[pygame.K_LEFT] or keys[pygame.K_a]) and player_rect.left > 0:
                self.dx = -1 * self.size
                self.dy = 0
            elif (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and player_rect.right < WINDOW_WIDTH:
                self.dx = 1 * self.size
                self.dy = 0
            else:
                self.dx = 0
    ...
    def reset_player(self):
        '''Reset player values for a neww round on the client side'''
        ...
        self.status_message = "Ready for the game, waiting for other players"
...
class Game():
    ...
    def __init__(self, connection, player, total_players):
        ...
        #Set the player count to play number - 1, we haven't recieved any game state data yet
    ...
    def send_player_info(self):
        ...
        #Create a dictinoray with items to send
        player_info = {
    ...
    def recieve_game_state(self):
        '''Recieve ALL info about ALL players from the server during the game'''
        ...
        #Process the game state informaation
    ...
    def draw(self):
        '''Draw the game and all game assets to the playspace'''
        ...
        #Create a list to hold all score
        current_scores = []
        
        #Loop thrpugh each players current state in the game_state
        for player in self.game_state:
        ...
        #Draw a magenta outline around CURRENT player to highlight
...
#Create a connection and get game windows information from the server.
